		...
		tparams, dpm_history_params hparams, char* fwl)
{
    ...
    psm_time_t history_active[DPM_HIST_WIND_SIZE];
    psm_time_t history_inactive[DPM_HIST_WIND_SIZE];
    int n_tran_total;
    int next_work_item;
    int num_work_items;
    int no_inactive = 0;
    ...
    n_tran_total = 0;
    next_work_item = 0;
    dpm_init_history(history_active);
    dpm_init_history(history_inactive);
    ...
    while (next_work_item < num_work_items) {
        ...
        for(int i=0; i< 11; i++)
        printf("%f  ", history_inactive[i]);
        printf("\n");

        while(t_curr < work_queue[next_work_item].arrival) {
            if (!dpm_decide_state(&curr_state, prev_state, t_curr, t_inactive_start, history_inactive, history_active, sel_policy, tparams, hparams)) {
        ...
        dpm_update_history(history_inactive, t_curr - t_inactive_start);
        ...
        while(next_work_item < num_work_items && t_curr >= work_queue[next_work_item].arrival) {
            if (no_inactive == 1) {
                dpm_update_history(history_inactive, 0);
            }
        ...
        dpm_update_history(history_active, t_curr - t_active_start);
    }
    free(work_queue);
...
int dpm_decide_state(psm_state_t *next_state, psm_state_t prev_state, psm_time_t t_curr,
        psm_time_t t_inactive_start, psm_time_t *history_inactive,psm_time_t *history_active, dpm_policy_t policy,
...
void dpm_update_history(psm_time_t *h, psm_time_t new_window)
{
	for (int i=0; i<DPM_HIST_WIND_SIZE-1; i++){
		h[i] = h[i+1];
	}
	h[DPM_HIST_WIND_SIZE-1] = new_window;
}
